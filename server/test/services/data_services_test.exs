defmodule DevTool.DataServicesTest do
  @moduledoc """
  ...
  alias DevTool.{DataServices, Environment, User}
  ...
  describe "DataServices.create_1/1" do
    ...
    test "should create reference if refs and refBy id is valid", %{
      env_id: env_id,
      user_id: _user_id
    } do
  ...
  describe "DataServices.update_1/1" do
    ...
    test "should return error if update with invalid refs id", %{
      env_id: env_id,
      user_id: _user_id
    } do
      {:ok, _inserted_datastore} = Repo.insert(Datastore.new(env_id, %{"name" => "users"}))
    ...
    test "should return error if update with invalid ref_by id", %{
      env_id: env_id,
      user_id: _user_id
    } do
      {:ok, _inserted_datastore} = Repo.insert(Datastore.new(env_id, %{"name" => "team"}))
